 ...
 *
 * @uses grind_header_style()
 */
function grind_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'grind_custom_header_args', array(
		'default-image'      => '',
		...
		'flex-height'        => true,
		'flex-width'         => true,
		'wp-head-callback'   => 'grind_header_style',
	) ) );
}
add_action( 'after_setup_theme', 'grind_custom_header_setup' );

if ( ! function_exists( 'grind_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see grind_custom_header_setup().
	 */
	function grind_header_style() {
